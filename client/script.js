...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((result) => {
        ...
        result.data.forEach(elem => {
            ...
            newAnimal.textContent = elem
            document.querySelector('body').appendChild(newAnimal)
        });
    })
}
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/ackoof').then((result) => {
    console.log(result.data)
...
// CODE HERE
function makeQuery(){
    axios.get('http://localhost:3000/query-test/?query2&lemons').then((result) =>{
        console.log(result.data)
    })
}
...
function createFood(e){
    e.preventDefault()
    let foodInput = document.querySelector('#food-form')
    let body = {
    ...
    axios.post('http://localhost:3000/food', body).then((result) => {
        ...
        result.data.forEach(elem => {
            ...
            newFood.textContent = elem
            document.querySelector('body').appendChild(newFood)
        });

        //or, for just the newest addition
        // let newfood = document.createElement('p')
        // newfood.textContent = result.data[result.data.length - 1]
        // document.querySelector('body').appendChild(newfood)
